...
const getAllBuyers = async (req, res, next) => {
  try {
    const result = await mongodb.getDb().db('homes').collection('Buyers').find();
...
const createBuyer = async (req, res) => {
  try {
    ...
    const response = await mongodb.getDb().db('homes').collection('Buyers').insertOne(buyer);
...
const deleteBuyer = async (req, res) => {
  try {
    ...
    const userId = new ObjectId(req.params.id);
    const response = await mongodb.getDb().db('homes').collection('Buyers').remove({ _id: userId }, true);
    console.log(response);
